...
void WriteToFile(STUDENT student[],int stu__num,int course_num,long subjecttotal[],float subjectaver[],long studenttotal[],float studentaver[]);
void ReadFile(STUDENT student[],int stu__num,int course_num,long subjecttotal[],float subjectaver[],long studenttotal[],float studentaver[]);
...
int main()
{
    ...
    printf("Student achievement management system v5.0\n\n");
    STUDENT student[30];
    ...
    do
    {
        ...
        case 6:
            ...
            SelectRank(Descending,student.studentID,stu__num,course_num,student);
            Outpute(stu__num,course_num,studenttotal,studentaver,student);
            printf("---------------------------------------------------------------------------------------------\n");
            break;
        case 7:
        ...
        case 12:
            ...
            WriteToFile(student,stu__num,course_num,subjecttotal,subjectaver,studenttotal,studentaver);
        ...
        case 13:
            ...
            ReadFile(student,stu__num,course_num,subjecttotal,subjectaver,studenttotal,studentaver);
            Outpute(stu__num,course_num,studenttotal,studentaver,student);
            printf("----------------------------------------------------------------------------------------------\n");
            break;
        case 0:
...
/**********************************
** 函数名:Rankname
** 输 入:STUDENT student[],int stu__num,int course_num,long subjecttotal[],float subjectaver[],long studenttotal[],float studentaver[]
** student【】结构体
** 功能描述:将所有信息存入student.txt中。
** 调用模块:
** 作 者:袁振洋
** 日 期:11.11
** 版本5.0
...
void WriteToFile(STUDENT student[],int stu__num,int course_num,long subjecttotal[],float subjectaver[],long studenttotal[],float studentaver[])
{
    ...
    fwrite(student,sizeof(STUDENT),stu__num,fp);
    fprintf(fp,"%d%d",stu__num,course_num);
    for(i=0; i<=stu__num-1; i++)
    {
        fprintf(fp,"%ld %f",studenttotal[i],studentaver[i]);
    }
    for(i2=0; i2<=course_num-1; i2++)
    {
        fprintf(fp,"%ld %f",subjecttotal[i2],subjectaver[i2]);
    }
    fclose(fp);
}
/**********************************
...
void ReadFile(STUDENT student[],int stu__num,int course_num,long subjecttotal[],float subjectaver[],long studenttotal[],float studentaver[])
{
    ...
    fread(student,sizeof(STUDENT),stu__num,fp);
    fscanf(fp,"%d%d",&stu__num,&course_num);
    for(i=0; i<=stu__num-1; i++)
    {
        fscanf(fp,"%ld %f",&studenttotal[i],&studentaver[i]);
    }
    for(i2=0; i2<=course_num-1; i2++)
    {
        fscanf(fp,"%ld %f",&subjecttotal[i2],&subjectaver[i2]);
    }
    fclose(fp);
